...
const push = (selfOrCompId, screen) => {
  console.log(
    "compId: ",
    selfOrCompId,
    get(selfOrCompId, "props.componentId", selfOrCompId)
  );
...
const pushExternalComponent = (self, name, passProps) =>
...
const showModal = screen =>
  Navigation.showModal(isString(screen) ? stack(screen) : screen);

const dismissModal = selfOrCompId =>
...
const mergeOptions = (selfOrCompId, options) =>
  Navigation.mergeOptions(compId(selfOrCompId), options);

const setStackRoot = (self, root) =>
